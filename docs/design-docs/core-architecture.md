...
│  - MafIntentRecognizer (意图识别器) ✅ 已实现                │
│  - MafTaskOrchestrator (任务编排器) ✅ 已实现 (callback模式) │
│  - MafAgentMatcher (Agent路由匹配) ✅ 已实现                 │
│  - DialogStateManager (多轮对话) ✅ 已实现                   │
│  - DefaultRagPipeline (RAG管道) ✅ 已实现                    │
