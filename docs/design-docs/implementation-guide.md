...
│   │   ├── NLP/                                # 意图识别 + 实体抽取
│   │   │   ├── MafIntentRecognizer.cs          # 委托到 HybridIntentRecognizer
│   │   │   ├── RuleBasedIntentRecognizer.cs    # 规则关键词意图识别
│   │   │   ├── MafEntityExtractor.cs           # 实体抽取器
│   │   │   └── IntentDrivenEntityExtractor.cs  # 意图驱动实体抽取
│   │   ├── Orchestration/
│   │   │   ├── MafTaskOrchestrator.cs          # 任务编排器 (callback模式)
│   │   │   ├── MafAgentMatcher.cs              # Agent 路由匹配
│   │   │   └── PersistentTaskOrchestrator.cs   # 持久化装饰器
│   │   ├── Dialog/
│   │   │   └── DialogStateManager.cs           # 多轮对话状态管理
│   │   ├── RAG/
│   │   │   └── DefaultRagPipeline.cs           # RAG 管道实现
│   │   ├── Registry/
│   │   │   └── LlmAgentRegistry.cs             # MafAiAgentRegistry 实现
│   │   ├── Resilience/
│   │   │   ├── DegradationManager.cs           # 降级管理器
│   │   │   └── MafRetryPolicy.cs
│   │   ├── Factory/
│   │   │   └── LlmAgentFactory.cs              # LLM工厂
│   │   ├── Monitoring/                         # Prometheus + 分布式追踪
│   │   ├── RealTime/                           # SignalR 实时推送
│   │   ├── Session/                            # 会话管理
│   │   └── Storage/                            # 分层存储
│   │
